    ...
    >
      {items.map((item, idx) => (
        <a
          ...
          onClick={onItemClick}
          className='relative px-4 py-2 text-neutral-600 dark:text-neutral-300 cyber:text-neutral-300'
          key={`link-${idx}`}
          href={item.link}
        >
          {hovered === idx && (
            <motion.div
              layoutId='hovered'
              className='absolute inset-0 h-full w-full rounded-full bg-gray-100 dark:bg-neutral-800 cyber:bg-neutral-800'
            />
          )}
